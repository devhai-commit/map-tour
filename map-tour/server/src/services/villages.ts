...
const VILLAGE_NAMES_BY_SLUG: Record<string, string> = {
  'lang-uoc-le': 'Làng Ước Lễ',
};
...
export async function findVillageDetailsBySlug(slug: string) {
  const villageName = VILLAGE_NAMES_BY_SLUG[slug];
  if (!villageName) return null;

  const villageResult = await pool.query<VillageRow>(
    `SELECT v.id, v.name, v.aliases, v.admin_location, v.google_maps_link,
